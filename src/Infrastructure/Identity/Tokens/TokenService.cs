...
public class TokenService : ITokenService
{
    ...
    public async Task<TokenResponse> LoginAsync(TokenRequest request)
    {
        ...
        return await GenerateTokenAndUpdateUserAsync(userInDb, request.RememberMe);
    }
    ...
    public async Task<TokenResponse> RefreshTokenAsync(RefreshTokenRequest request)
    {
        ...
        return await GenerateTokenAndUpdateUserAsync(userInDb, false);
    }
    ...
    private async Task<TokenResponse> GenerateTokenAndUpdateUserAsync(ApplicationUser user, bool rememberMe)
    {
        var newJwt = await GenerateToken(user);

        user.RefreshToken = GenerateRefreshToken();
        user.RefreshTokenExpiryTime = rememberMe ? DateTime.UtcNow.AddMonths(3) : DateTime.UtcNow.AddDays(_jwtSettings.RefreshTokenExpiryTimeInDays);
