...
public class DeactivateTenantCommandHandler : IRequestHandler<DeactivateTenantCommand, IResponseWrapper>
{
    ...
    {
        var tenantId = await _tenantService.DeactivateAsync(request.TenantId, cancellationToken);
        return await ResponseWrapper<string>.SuccessAsync(data: tenantId, "Tenant de-activativation successful.");
    }
}